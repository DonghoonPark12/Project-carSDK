...
static int Frame2Ipl(IplImage* img, IplImage* imgResult, IplImage* imgCenter)
{
	...
	static short angle = 1600;
	//	int cnt=0,width_temp=0, width_count=0,temp=0;;
	...
	//영상 가운데를 중심으로 양쪽 차선 찾기
	for(j = 80; j < resHeight; j++)
	{
		k=160;
		check_side = 0;
		left=0;
	...
	/*
	   ...
	   temp = temp+k;
	   while( imgResult->imageData[j*imgResult->widthStep + k] != (char)0 )
	   {
	   k++;
	   }
	   ...
	   if( (width_temp - temp) >20 | (width_temp - temp) < -20)
	   {
	   temp = width_temp + 1;
	   }

	   width_temp = temp;
	   imgCenter->imageData[j*imgCenter->widthStep + temp] = 255;
	   }
	   else
	   {
	   temp = temp/2;
...
static unsigned int CaptureThread(void *params)
{
	...
	while((ctx->timeNotCount? (SubTime(&t1, &t2)): ((unsigned int)i < ctx->last)) && !stop)
	{
		...
		if(i%30 == 0)                        // once in three loop = 10 Hz
...
void *ControlThread(void *unused)
{
	...
	imgY = cvCreateImage(cvGetSize(imgOrigin), IPL_DEPTH_8U, 1);
	imgU = cvCreateImage(cvGetSize(imgOrigin), IPL_DEPTH_8U, 1);
	imgV = cvCreateImage(cvGetSize(imgOrigin), IPL_DEPTH_8U, 1);
	...
	while(1)
	{
		...
		Frame2Ipl(imgOrigin,imgResult,imgCenter); // save image to IplImage structure & resize image from 720x480 to 320x240
		...
		//	cvSplit(imgOrigin, imgY, imgU, imgV, NULL);
		//	cvCvtColor(imgOrigin, RGB , CV_YUV2BGR);

		//    sprintf(fileName, "captureImage/imgYuv%d.png", i);
		//	sprintf(fileName2, "captureImage/imgY%d.png", i);
		//	sprintf(fileName3, "captureImage/imgU%d.png", i);
		//	sprintf(fileName4, "captureImage/imgV%d.png", i);
		sprintf(fileName5, "captureImage/imgResult%d.png", i);
		//	sprintf(fileName6, "captureImage/imgRGB%d.png", i);
		sprintf(fileName7, "captureImage/imgCenter%d.png", i);

		//        cvSaveImage(fileName, imgOrigin, 0);
		//	cvSaveImage(fileName2, imgY, 0);
		//	cvSaveImage(fileName3, imgU, 0);
		//	cvSaveImage(fileName4, imgV, 0);
		cvSaveImage(fileName5, imgResult, 0);
		//	cvSaveImage(fileName6, RGB, 0);
